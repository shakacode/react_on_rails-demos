...
module DemoScripts
  ...
  class DependencySwapper < DemoManager
    ...
    def validate_gem_paths(paths)
      ...
      # Only expand paths, don't validate existence here
      # Path existence is validated in validate_local_paths! which is only called during swap!
    ...
    def validate_local_paths!
      ...
      gem_paths.each do |gem_name, path|
        ...
        missing_paths << "#{gem_name}: #{path}"
      end

      return if missing_paths.empty?

      error_msg = "The path `#{missing_paths.first.split(': ').last}` does not exist.\n\n"
